...
namespace PageConstructor.Infrastructure.Common.Services;
...
public class FileUploadService : IFileUploadService
{
    ...
    public async ValueTask<string> UploadComponentPreviewAsync(IFormFile file)
    {
        var uploadsPath = Path.Combine(_env.WebRootPath ?? "wwwroot", "uploads", "components");
        Directory.CreateDirectory(uploadsPath);
        ...
        var request = _http.HttpContext?.Request;
        return $"{request?.Scheme}://{request?.Host}/uploads/components/{fileName}";
    }
}
